...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        body = expressions.rest  # a bunch of expressions
...
def do_quote_form(expressions, env):
    ...
    # BEGIN PROBLEM 5
    return expressions.first  # 最简单的一集
    # END PROBLEM 5
...
def do_lambda_form(expressions, env):
    ...
    # BEGIN PROBLEM 7
    exprs = expressions.rest
    return LambdaProcedure(formals, exprs, env)
    # END PROBLEM 7
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if is_scheme_true(test):
            # BEGIN PROBLEM 13
            if clause.rest is nil:  # without a result sub-expression
                return test
...
def make_let_frame(bindings, env):
    ...
    while binding is not nil:
        ...
        val = scheme_eval(binding.first.rest.first, env)  # 最绕的一集
        vals = Pair(val, vals)
